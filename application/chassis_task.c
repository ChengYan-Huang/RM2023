...
//        cos_yaw = arm_cos_f32(-chassis_move_control->revolve_angle);
        ...
        chassis_move_control->chassis_relative_angle_set = rad_format(angle_set)+chassis_move_control->chassis_yaw_motor->relative_angle;
				        ...
				        //计算旋转PID角速度
        chassis_move_control->wz_set = -2.5*(-PID_calc(&chassis_move_control->chassis_angle_pid, chassis_move_control->chassis_yaw_motor->relative_angle, chassis_move_control->chassis_relative_angle_set));
...
//				chassis_move_control->wz_set = CHASSIS_WZ_RC_SEN * angle_set;
    ...
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_NO_FOLLOW_YAW)//jl
    {
        //"angle_set" is rotation speed set-point
        //“angle_set” 是旋转速度控制
        chassis_move_control->wz_set = angle_set;
        chassis_move_control->vx_set = fp32_constrain(vx_set, -0.9f, 0.9f);
        chassis_move_control->vy_set = fp32_constrain(vy_set, -0.9f, 0.9f);
    }
