...
//pitch是围绕X轴旋转，抬头为正，反之为负
//yaw是围绕Y轴旋转，右偏航为正，反之为负
...
void CAN_cmd_fric(int16_t left, int16_t right, int16_t rev1, int16_t rev2)
{
    uint32_t send_mail_box;
    fric_tx_message.StdId = 0x1FF;
    fric_tx_message.IDE = CAN_ID_STD;
    fric_tx_message.RTR = CAN_RTR_DATA;
    fric_tx_message.DLC = 0x08;
    fric_can_send_data[0] = (left >> 8); 
    fric_can_send_data[1] = left;        
    fric_can_send_data[2] = (right >> 8);
    fric_can_send_data[3] = right;
...
void CAN_send_msg(uint16_t power)
{
    uint32_t send_mail_box;
    chassis_tx_message.StdId = CAN_ID;//0x210
    chassis_tx_message.IDE = CAN_ID_STD;
    chassis_tx_message.RTR = CAN_RTR_DATA;
    chassis_tx_message.DLC = 0x08;
    chassis_can_send_data[0] = power >> 8;
    chassis_can_send_data[1] = power;
